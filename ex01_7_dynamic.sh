...
# cases/test_cases_ex011_7.txtからランダムに5つのテストケースを選択
mapfile -t selected_cases < <(shuf -n 5 cases/test_cases_ex01_7.txt)
...
for test_case in "${selected_cases[@]}"; do
    ...
    echo $inputs | ./ex01_7 | sed 's/Input 2 numbers: //' | tr -d '\n ' > "$temp_output"
    echo $inputs | ./ex01_7 | sed 's/Input 2 numbers: //' | tr -d '\n' > "$temp_output2"
    echo $inputs | ./ex01_7 > /dev/null 2> "$temp_error"
